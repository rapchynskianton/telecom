...
function initSelectElement(htmlElementId, jsonArray,valueKey) {
	...
	$.each(jsonArray, function(key, value) {
		$('#' + htmlElementId).append($("<option></option>").attr("value", value.id).text(value[valueKey]));
	});
}
...
function initComboboxes(contextUrl) {
	...
	// местом встави в JSP)
	$.get(contextUrl + "/tariffPlan/nameAttribute", function(nameAttributeArray) {

		// получив список стран - инициализируем соответствующий комбик
		initSelectElement('nameAttribute', nameAttributeArray,'name');
		...
		// запрос
		// на сервер
		$("#nameAttribute").change(function() {
			var selectedId = $(this).val();
			$.get(contextUrl + "/tariffPlan/valueAttribute?valueAttributeId=" + selectedId, function(valueAttributeArray) {
				initSelectElement('valueAttribute', valueAttributeArray,'value');
			})
		});
